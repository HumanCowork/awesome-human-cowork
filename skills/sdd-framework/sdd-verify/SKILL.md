...
### Step 4b: Check UI & Figma Compliance (Visual Components)

If the change involves UI components, verify that their styling matches the Figma design constraints specified in `design.md` or `spec.md`.

```
FOR EACH UI COMPONENT modified:
├── Read the constraints from the design/spec documents (paddings, margins, typography, layout)
├── Verify the code uses centralized design tokens (e.g., `<Container>`, theme variables) rather than hardcoded arbitrary utility classes (`px-4`, `p-6`)
├── Verify that breakpoints and responsive behavior match the constraints
└── Flag: CRITICAL if hardcoded utility classes violate the design system and cause responsive breakage (like overflow)
```
